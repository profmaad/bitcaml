...
let check_random_folder storage =
  let path = String.concat "/" (storage.root_path :: (List.map (Printf.sprintf "%02x") (Utils.random_list 255 storage.folder_levels))) in
...
let path_of_hash storage hash =
  ...
  in
  storage.root_path ^ (String.concat "/" (path_of_hash_ hash storage.folder_levels)) ^ "/" ^ hash
;;
